...
/**
 * ══════════════════════════════════════════════════════════════════
 *  PptEditorScreen — WebView-এ assets/officeapp/ppt/index.html লোড
 *  করে, JS bridge দিয়ে সম্পূর্ণ presentation state (স্লাইড + এলিমেন্ট)
 *  JSON হিসেবে get/set করে OfficeFileStore-এ সেভ করে।
 * ══════════════════════════════════════════════════════════════════
 */
...
fun PptEditorScreen(
    ...
    fun saveNow(showToast: Boolean = true) {
        ...
        wv.evaluateJavascript("(function(){return window.getPresentationState();})()") { raw ->
            val json = try {
    ...
    fun printSlides() {
        ...
        val jobName = "${file.title} - SmartStudy PowerPoint"
        ...
        topBar = {
            TopAppBar(
                ...
                actions = {
                    ...
                    IconButton(onClick = { printSlides() }) { Icon(Icons.Default.Print, contentDescription = "Print / Export PDF") }
                    Box {
                        ...
                        DropdownMenu(expanded = showMenu, onDismissRequest = { showMenu = false }) {
                            ...
                            DropdownMenuItem(
                                ...
                                onClick = { showMenu = false; printSlides() }
                            )
                        }
                    }
                }
            ...
            AndroidView(
                ...
                factory = { ctx ->
                    WebView(ctx).apply {
                        ...
                        addJavascriptInterface(object {
                            ...
                            fun onEditorReady() {
                                webViewSelf.post {
                                    ...
                                    webViewSelf.evaluateJavascript("window.setPresentationState($escaped)", null)
                                }
                            }
                            @JavascriptInterface
                        ...
                        loadUrl("file:///android_asset/officeapp/ppt/index.html")
                        webViewRef = this
                    }
                }
            )
